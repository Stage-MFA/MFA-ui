...
export default function MaintenancesList() {
  ...
  const filteredMaintenances = maintenances.filter((mnt) => {
    ...
    const req = intervention
      ...
      : undefined;
    const matchesTechnician = currentUser
      ? intervention?.usersId === currentUser.id
      : true;
    const matchesSearch =
    ...
    const matchesDate = selectedDate
      ...
      : true;
    return matchesTechnician && matchesSearch && matchesStatus && matchesDate;
  });
  ...
  const handleCreate = (id: number) => {
    router.prefetch(
      `/technicien-ministere/maintenance/edit?maintenanceId=${id}`,
    );
